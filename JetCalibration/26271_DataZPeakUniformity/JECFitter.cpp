...
public:
   AllEvent();
   AllEvent(string FileNameMC, string FileNameData, double MinimumE = 5, int MaximumN = 3, double Percentage = 1, double ThetaMin = 0.15, double ThetaMax = 0.85);
   void Initialize(string FileNameMC, string FileNameData, double MinimumE = 5, int MaximumN = 3, double Percentage = 1, double ThetaMin = 0.15, double ThetaMax = 0.85);
   ...
   double GetJEC(double Energy, double Theta, const double *Parameters) const;
   double Likelihood(const double *Parameters);
   vector<pair<double, double>> DoFit();
};
...
AllEvent::AllEvent(string FileNameMC, string FileNameData, double MinimumE, int MaximumN, double Percentage, double ThetaMin, double ThetaMax)
{
   Initialize(FileNameMC, FileNameData, MinimumE, MaximumN, Percentage, ThetaMin, ThetaMax);
}

void AllEvent::Initialize(string FileNameMC, string FileNameData, double MinimumE, int MaximumN, double Percentage, double ThetaMin, double ThetaMax)
{
   ECut = MinimumE;
   NJet = MaximumN;
...
void AllEvent::ReadFile(vector<Event> &E, string FileName, double Percentage)
{
   ...
   for(int i = 0; i < EntryCount; i++)
   {
      ...
      for(int j = 0; j < N; j++)
      {
         if(JetTheta[j] < MinTheta * M_PI || JetTheta[j] > MaxTheta * M_PI)
            JetPT[j] = 0;
...
map<int, vector<double>> AllEvent::GetMass(const vector<Event> &E, bool ApplyJEC, const double *Parameters) const
{
   ...
   for(int i = 0; i < (int)E.size(); i++)
   {
      ...
      for(int j = 0; j < E[i].N; j++)
      {
         ...
         double Momentum = sqrt(E[i].PX[j] * E[i].PX[j] + E[i].PY[j] * E[i].PY[j] + E[i].PZ[j] * E[i].PZ[j]);
         double Theta = acos(E[i].PZ[j] / Momentum);
         
         double JEC = (ApplyJEC ? GetJEC(Energy, Theta, Parameters) : 1);
         if(Energy * JEC < ECut)
            continue;
         ...
         if(Energy * JEC > LeadingE || LeadingE < 0)
         {
            LeadingE = Energy * JEC;
            LeadingTheta = Theta;
         }
...
double AllEvent::GetJEC(double Energy, double Theta, const double *Parameters) const
{
   ...
   if(FunctionType == Linear)
      return Overall * (1 + Parameters[6] * Energy);
   if(FunctionType == Quadratic)
      return Overall * (1 + Parameters[6] * Energy + Parameters[7] * Energy * Energy);
   if(FunctionType == Cubic)
      return Overall * (1 + Parameters[6] * Energy + Parameters[7] * Energy * Energy + Parameters[8] * Energy * Energy * Energy);
   if(FunctionType == Quartic)
      return Overall * (1 + Parameters[6] * Energy + Parameters[7] * Energy * Energy + Parameters[8] * Energy * Energy * Energy + Parameters[9] * Energy * Energy * Energy * Energy);
   if(FunctionType == Cintic)
      return Overall * (1 + Parameters[6] * Energy + Parameters[7] * Energy * Energy + Parameters[8] * Energy * Energy * Energy + Parameters[9] * Energy * Energy * Energy * Energy + Parameters[10] * Energy * Energy * Energy * Energy * Energy);
...
vector<pair<double, double>> AllEvent::DoFit()
{
   ...
   if(MinTheta < 0.25 || MaxTheta > 0.75)   Minimizer->SetLimitedVariable(0, "JECT0", 1.000, 0.003, 0.5, 1.5);
   else                                     Minimizer->SetFixedVariable(0, "JECT0", 1.000);
   if(MinTheta < 0.30 || MaxTheta > 0.70)   Minimizer->SetLimitedVariable(1, "JECT1", 1.000, 0.003, 0.5, 1.5);
   else                                     Minimizer->SetFixedVariable(1, "JECT1", 1.000);
   Minimizer->SetLimitedVariable(2, "JECT2", 1.000, 0.003, 0.5, 1.5);
   Minimizer->SetLimitedVariable(3, "JECT3", 1.000, 0.003, 0.5, 1.5);
   Minimizer->SetLimitedVariable(4, "JECT4", 1.000, 0.003, 0.5, 1.5);
   Minimizer->SetLimitedVariable(5, "JECT5", 1.000, 0.003, 0.5, 1.5);

   if(N > 6)    Minimizer->SetLimitedVariable(6, "JEC1", -0.00005, 0.001, -1.0, 1.0);
   ...
   const double *E = Minimizer->Errors();
...
int main(int argc, char *argv[])
{
   ...
   double ThetaMin     = CL.GetDouble("ThetaMin", 0.15);
   double ThetaMax     = CL.GetDouble("ThetaMin", 0.85);

   AllEvent Events(FileNameMC, FileNameData, ECut, NJet, Percentage, ThetaMin, ThetaMax);
   
   Events.FunctionType = AllEvent::Const;
   vector<pair<double, double>> VParameter0 = Events.DoFit();
   Events.FunctionType = AllEvent::Linear;
   vector<pair<double, double>> VParameter1 = Events.DoFit();
   // Events.FunctionType = AllEvent::Quadratic;
   // vector<pair<double, double>> Parameter2 = Events.DoFit();
   // const double *Parameter2 = Events.DoFit();
   // vector<pair<double, double>> Parameter3 = Events.DoFit();
   // const double *Parameter3 = Events.DoFit();
   // vector<pair<double, double>> Parameter4 = Events.DoFit();
   // Events.FunctionType = AllEvent::Cintic;
   // vector<pair<double, double>> Parameter5 = Events.DoFit();

   double Parameter0[6], Parameter1[7];
   for(int i = 0; i < 6; i++)
      Parameter0[i] = VParameter0[i].first;
   for(int i = 0; i < 7; i++)
      Parameter1[i] = VParameter1[i].first;

   DataHelper DHFile(DHFileName);

   DHFile[State]["ECut"]     = ECut;
   DHFile[State]["NJet"]     = NJet;
   DHFile[State]["ThetaMin"] = ThetaMin;
   DHFile[State]["ThetaMax"] = ThetaMax;

   DHFile[State]["P0T0"]      = VParameter0[0].first;
   DHFile[State]["P0T1"]      = VParameter0[1].first;
   DHFile[State]["P0T2"]      = VParameter0[2].first;
   DHFile[State]["P0T3"]      = VParameter0[3].first;
   DHFile[State]["P0T4"]      = VParameter0[4].first;
   DHFile[State]["P0T5"]      = VParameter0[5].first;
   DHFile[State]["P0ET0"]     = VParameter0[0].second;
   DHFile[State]["P0ET1"]     = VParameter0[1].second;
   DHFile[State]["P0ET2"]     = VParameter0[2].second;
   DHFile[State]["P0ET3"]     = VParameter0[3].second;
   DHFile[State]["P0ET4"]     = VParameter0[4].second;
   DHFile[State]["P0ET5"]     = VParameter0[5].second;
   
   DHFile[State]["P1T0"]      = VParameter1[0].first;
   DHFile[State]["P1T1"]      = VParameter1[1].first;
   DHFile[State]["P1T2"]      = VParameter1[2].first;
   DHFile[State]["P1T3"]      = VParameter1[3].first;
   DHFile[State]["P1T4"]      = VParameter1[4].first;
   DHFile[State]["P1T5"]      = VParameter1[5].first;
   DHFile[State]["P11"]       = VParameter1[6].first;
   DHFile[State]["P1ET0"]     = VParameter1[0].second;
   DHFile[State]["P1ET1"]     = VParameter1[1].second;
   DHFile[State]["P1ET2"]     = VParameter1[2].second;
   DHFile[State]["P1ET3"]     = VParameter1[3].second;
   DHFile[State]["P1ET4"]     = VParameter1[4].second;
   DHFile[State]["P1ET5"]     = VParameter1[5].second;
   DHFile[State]["P1E1"]      = VParameter1[6].second;
   ...
   Legend.AddEntry(&H11, "Linear, E = 5", "l");
   Legend.AddEntry(&H12, "Linear, E = 50", "l");
