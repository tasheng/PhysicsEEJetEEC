   ...
   for(int iE = 0; iE < EntryCount; iE++)
   {
      ...
      for(int i = 0; i < NParticle; i++)
      {
         for(int j = i + 1; j < NParticle; j++)
         {
            ...
            E1E2Gen[NPair] = GenE[i]*GenE[j];
            E1E2Reco[NPair] = RecoE[i]*RecoE[j];
...
double MetricAngle(FourVector A, FourVector B)
{
   ...
   double Eres = 0.85/sqrt(meanE); // take energy resolution based on the mean energy
   double AngleRes = (0.01*0.01); // take angular resolution based on the detector resolution
   double chi2metric = (Angle*Angle)/(AngleRes*AngleRes) + (Ediff*Ediff)/(Eres*Eres);
   return chi2metric; 
}
