...
int main(int argc, char *argv[]){
   ...
   for(int i = 0; i <= BinCount; i++)
   {
    ...
    for(int i = 0; i < N; i++){
      for(int j = 1; j < nIters+2; j++){
        std::cout << "On file " << i << " and iteration " << j << std::endl;
        Files[i]->ls(); 
        TH2D *hUnfolded; 
        if(j == nIters +1){
         hUnfolded = (TH2D*)Files[i]->Get("true_Theta");
        }
        else{
            hUnfolded = (TH2D *)Files[i]->Get(Form("Bayesian_Unfoldediter%d_Theta", j));
        }
        if(hUnfolded == NULL){
        ...
        H->SetMarkerColor(Colors[j-1]);
        H->SetMarkerStyle(20);
        H->SetLineColor(Colors[j-1]);
        H->SetLineWidth(2);
   ...
   string X, string Y, double WorldMin, double WorldMax, bool DoRatio, bool LogX)
{
   ...
   Latex.SetTextColor(kBlack);
   if(DoRatio)
      Latex.DrawLatex(MarginL * 0.3, MarginB + PadRHeight * 0.5, "Ratio to Archived MC");
   ...
   Latex.DrawLatex(0.01, 0.01, "Work-in-progress, 2024 Sept 22nd, Hannah Bossi Hard Probes");
